...
export default function EditOrderPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    let mounted = true
    orderService.getOrderById(id).then(o => { if (mounted) setForm(o) }).catch(e => { if (mounted) setError(e.message) }).finally(() => mounted && setLoading(false))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await orderService.updateOrder(id, form)
      router.push('/orders')
    } catch (err: any) {
